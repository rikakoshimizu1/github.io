...
original = copy.deepcopy(newArray)
...
for i in range(0, n):
    for j in range(0, m):
        ...
        # Count Number of Greens (1)
        # all the same for middle, left, and right
        countOriginalGreen = original[i][j] + original[i][(j - 1) % m] + original[i][(j + 1) % m]

        # If is the upper or lower edge
        if i == 0 | i == m:
            countOriginalGreen = countOriginalGreen + original[(i - 1) % m][(j - 1) % m] \
                         + original[(i - 1) % m][j] + original[(i + 1) % m][(j - 1) % m] + original[(i + 1) % m][j]
        # If is middle or left/right edge odd row
        if i % 2 == 1:
            countOriginalGreen = countOriginalGreen + original[(i - 1) % m][j] + original[(i - 1) % m][(j + 1) % m] \
                         + original[(i + 1) % m][j] + original[(i + 1) % m][(j + 1) % m]
        # If is middle or left/right edge even row
        elif i % 2 == 0:
            countOriginalGreen = countOriginalGreen + original[(i - 1) % m][(j - 1) % m] + original[(i - 1) % m][j] \
                         + original[(i + 1) % m][(j - 1) % m] + original[(i + 1) % m][j]
