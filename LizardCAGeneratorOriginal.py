...
def iterate(array, startingAge, maxAge):
    global countGreenArray, n, m, newArray, countGreenAroundBlack, countBlackAroundGreen, counter, numberBlack, numberGreen, numGreen
    if startingAge == maxAge:
        return

    # Create a temporary 2D array (Transformed)
    #tempArray = [[0 for i in range(a)] for j in range(b)]
    ...
    if startingAge <= 15:
        ...
        for i in range(0, n):
            for j in range(0, m):
                countGreenUpdated = 0
                countGreenUpdated = newArray[i][j] + newArray[i][(j - 1) % m] + newArray[(i + 1) % m][(j - 1) % m] + newArray[(i - 1) % m][
                                        j] + newArray[(i + 1) % m][j] + newArray[(i - 1) % m][(j + 1) % m] + newArray[i][
                                        (j + 1) % m]
        ...
        for i in range(0, n):
            for j in range(0, m):
                ...
                countGreen = 0
                # Count Number of Greens (1)
                countGreen = array[i][j] + array[i][(j - 1) % m] + array[(i + 1) % m][(j - 1) % m] + array[(i - 1) % m][
                    j] + array[(i + 1) % m][j] + array[(i - 1) % m][(j + 1) % m] + array[i][(j + 1) % m]
                if countGreen == 0 or countGreen == 1 or countGreen == 2:
                    # If center hex is black
                    if array[i][j] == 0:
                        # Change the new array to green
        ...
        for i in range(0, n):
            for j in range(0, m):
                countGreenUpdated = 0
                countGreenUpdated = newArray[i][j] + newArray[i][(j - 1) % m] + newArray[(i + 1) % m][(j - 1) % m] + \
                                    newArray[(i - 1) % m][
                                        j] + newArray[(i + 1) % m][j] + newArray[(i - 1) % m][(j + 1) % m] + newArray[i][
                                        (j + 1) % m]
                countGreenArray[i][j] = countGreenUpdated

    print(countGreenArray)

    numberBlack.append((a*b) - numGreen)
